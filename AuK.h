 ...
 * the new task.
 * 
 * mem_needed must contain the needed words (of 16 bits) for the task stack. 
 * 
 ...
 * to the context of the running task (all dsp registers). That must be 
 ...
 * the timer).
 * 
 * 2) Additionally, when the count_tic is executed (inside int TIMER-1 INT) 
 * it also temporally consumes 2 words (4 bytes) of stack space when return
 * address is saved and another 2 word (4 bytes) of a local integer variable.
 * 
 ...
 * 
 * Returns -2 if declared priority is equal or greather than max_TCB_indx or
 * lower than 0.
 * 
 ...
 * 
 * 
 * ATTENTION. Creating a new task must not be done while AuK is running. 
 * If it is necessary, it must be done in an exclusive way, even preventing 
 * the scheduler from working (temporarily disabling the timer 1 interrupt).
 * 
 */
...
/* Struct to manage chains of process identifiers ordered by priority and 
...
/* It decrements the counter of the semaphore. If the counter value was 
 ...
 * its execution. It makes easier to prevent the driver and interrupt 
 ...
 * specified interrupt before context change.*/
